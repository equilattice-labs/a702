...
async function loadModule({ configured = false, dependencies, configOverrides = {} } = {}) {
  const config = {
...
test('public evidence rejects Twitter/X hosts at exact domain boundaries', async () => {
  const module = await loadModule();
  for (const host of ['twitter.com', 'x.com', 't.co']) {
    for (const prefix of ['', 'www.', 'mobile.', 'research.sources.']) {
      for (const suffix of ['', '.', '..']) {
        for (const protocol of ['http:', 'https:']) {
          const uri = `${protocol}//${prefix}${host.toUpperCase()}${suffix}/evidence?source=research`;
          check.equal(module.safeExternalUrl(uri), '', `Block prohibited host: ${uri}`);
        }
      }
    }
  }
  for (const uri of ['https://%78.com/source', 'https://x。com/source', 'https://x.com:443/source', 'https://t.co:8080/source']) {
    check.equal(module.safeExternalUrl(uri), '', `URL normalization cannot bypass the host policy: ${uri}`);
  }
  for (const uri of ['https://notx.com/source', 'https://x.com.example.org/source', 'https://twitter.com.example.org/source', 'https://t.company/source', 'https://example.org/x.com?q=twitter.com']) {
    check.equal(module.safeExternalUrl(uri), new URL(uri).href, `Keep a legitimate unrelated host: ${uri}`);
  }
  const form = { title: 'A research question', category: 'Market structure', uri: 'https://X.com./brief', reward: '0.01', deadline: new Date(Date.now() + 86400000).toISOString() };
  check.ok(module.validateMissionForm(form).errors.uri, 'Publishing rejects prohibited brief destinations');
  const blockedExplorer = await instantiate(await loadModule({ configOverrides: { EXPLORER_URL: 'https://explorer.X.COM.' } }));
  check.equal(blockedExplorer.explorerLink(`0x${'a'.repeat(64)}`), '', 'Dynamic explorer links respect the host policy');
  const validExplorer = await instantiate(module);
  check.equal(validExplorer.explorerLink(`0x${'a'.repeat(64)}`), `https://explorer.example/tx/0x${'a'.repeat(64)}`);
});
...
test('multi-generation bookmarks preserve namespaces, removals and read-only storage', async context => {
  ...
  let storage = new Map([
    ['siftlane-saved', JSON.stringify([`${namespace}0`, 'sample-2', anotherDeployment, `${namespace}0`])],
    ['citeward-saved', '["sample-3"]'],
    ['civiquill-saved', '["sample-4"]'],
    ['proofora-saved', '[1,3]'],
  ]);
  ...
  check.ok(state.notice.value.includes('Removed from your saved list.'), 'Read-only storage still reports the actual bookmark action');
  check.ok(state.notice.value.includes('for this visit only'));
});
...
test('bookmark undo restores Saved rows, preserves namespaces and expires with its own notification', async context => {
  ...
  setGlobalForTest(context, 'localStorage', {
    getItem: key => storage.get(key) ?? null,
    setItem: (key, value) => { if (rejectWrites) throw new Error('Read-only storage'); storage.set(key, value); },
  });
  context.mock.method(globalThis, 'setTimeout', (callback, delay) => {
    check.equal(delay, 7000, 'Undo is available for the current notification lifetime');
    timers.set(++nextTimer, callback);
    return nextTimer;
  });
  context.mock.method(globalThis, 'clearTimeout', id => timers.delete(id));
  const state = await instantiate(await loadModule());
  const [first, second] = state.missionItems.value;
  state.saveMission(first);
  check.equal(state.notice.value, 'Saved to your list.');
  check.equal(state.noticeKind.value, 'bookmark');
  check.equal(state.canUndoSavedChange.value, true);
  const staleTimeout = timers.get(nextTimer);
  state.saveMission(second);
  staleTimeout();
  check.equal(state.canUndoSavedChange.value, true, 'A replaced notification timer cannot expire the new Undo action');
  check.equal(state.undoSavedChange(), true);
  check.equal(state.isSaved(second), false, 'Undo reverses the most recent save');
  check.equal(state.isSaved(first), true, 'Undo preserves earlier bookmark changes');
  check.deepEqual(state.previewSaved.value, [foreignKey, first.key], 'Undo never replaces the entire saved list or alters another deployment');
  check.equal(state.canUndoSavedChange.value, false, 'A completed Undo cannot be repeated');
  state.activeView.value = 'Saved';
  state.saveMission(first);
  check.equal(state.filtered.value.length, 0, 'Removing the last saved mission removes its row immediately');
  check.equal(state.notice.value, 'Removed from your saved list.');
  check.equal(state.undoSavedChange(), true);
  ...
  check.equal(state.undoSavedChange(), false);
  check.equal(state.isSaved(first), false);
  state.saveMission(first);
  timers.get(nextTimer)();
  check.equal(state.notice.value, '', 'Expiry closes the current notification');
  check.equal(state.canUndoSavedChange.value, false);
  check.equal(state.undoSavedChange(), false, 'Expired actions cannot change bookmarks');
  check.equal(state.isSaved(first), true);
  state.saveMission(first);
  state.notify('Transaction confirmed on Robinhood Chain Testnet.', { kind: 'transaction' });
  check.equal(state.noticeKind.value, 'transaction');
  check.equal(state.canUndoSavedChange.value, false, 'A transaction notification never inherits bookmark Undo');
  check.equal(state.undoSavedChange(), false);
  check.equal(state.isSaved(first), false, 'Transaction notifications cannot restore old bookmarks');
  rejectWrites = true;
  state.saveMission(first);
  check.equal(state.isSaved(first), true, 'Read-only storage retains a save for the current visit');
  check.ok(state.notice.value.includes('for this visit only'));
...
test('mocked wallet transactions: all six actions, permissions, precision and state refresh', async context => {
  ...
  state.saveMission(state.selected.value);
  check.equal(state.canUndoSavedChange.value, true);
  state.contribution.value = '0.000000000000000001';
  check.equal(await state.transact('fund'), true);
  check.equal(state.noticeKind.value, 'transaction');
  check.equal(state.canUndoSavedChange.value, false, 'Confirmed chain actions replace the bookmark notification without offering Undo');
  check.equal(state.undoSavedChange(), false, 'Undo cannot invoke or reverse a wallet transaction');
  check.equal(state.isSaved(state.selected.value), true, 'A transaction leaves the previously saved bookmark intact');
  ...
  state.evidence.value = 'https://T.CO./research';
  check.equal(await state.transact('submit'), false, 'Prohibited social evidence is rejected before wallet interaction');
  check.equal(contributed.length, 1);
  check.ok(state.formErrors.value.evidence);
